...
def test_algo_nested_parens():
    alg = Algorithm("B (R (U F2)3 R')2 F (R U R' U')")
    assert isinstance(alg, Algorithm)
    assert str(alg) == "B R U F2 U F2 U F2 U F2 U F2 U F2 R' F R U R' U'"
...
def test_algo_condensed():
    ...
    condensed = s.replace(' ','')
    assert str(Algorithm(condensed)) == s

def test_algo_wide_moves():
    alg = Algorithm("Rw U Rw'")
    assert str(alg) == "Rw U Rw'"

def test_algo_xyz_move():
    alg = Algorithm("(x2 y) R' F D2 R D'")
...
def test_algo_invalid_mult():
    with pytest.raises(InvalidAlgorithmError):
        Algorithm("R U2 (D x)0 R ")
    with pytest.raises(InvalidAlgorithmError):
        Algorithm("R U2 (D x)000 R ")

def test_algo_mismatched_paren():
    with pytest.raises(InvalidAlgorithmError):
        Algorithm("(R U")
    with pytest.raises(InvalidAlgorithmError):
        Algorithm("R U2 (D ) R ) B ( F ( L )")